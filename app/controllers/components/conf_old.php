...
class ConfComponent extends Object
{
    ...
    function startup(&$controller) {
        ...
        if(!$this->started) {
            ...
            $this->started = true;
            if(App::Import('Model','ConfigCategory')) {
                ...
                $ccs = $this->ccModel->find('all'); //findAll() doesn't work for cake 1.3 ..
    ...
    // mugabo interpretation of get means fetch 
    
    function fetch($categorie) {
    	$categorieLower=strtolower($categorie);
    	$parametres=array();
      if(App::Import('Model','ConfigCategory')) {
       	$this->ccModel = & new ConfigCategory();
    		$this->donnees=$this->ccModel->find('all');
    		foreach($this->donnees as $donnees) {
    			if($donnees['ConfigCategory']['name']==$categorieLower) {
    				foreach($donnees['Config'] as $config) {
    					$parametres[$categorie][$config['key']]=$config['value'];
    				}
    				break;
    			}	
    		}
    ...
    function setCat($cat,$data,$addCat = false,$addKeys = false) {
        foreach($data as $k => $v) {
            ...
            if(!$this->set($cat.'.'.$k,$v,null,$addCat,$addKeys)) { //il y avait une erreur oublie de null
                return false;
            }
        }
        return true;
    ...
    function setBatch($data,$addCats = false,$addKeys = false) {
        foreach($data as $cat => $catData) {
            foreach($catData as $k => $v) {
                // not very useful, the right way is to use a transaction
                if(!$this->set($cat.'.'.$k,$v,null,$addCats,$addKeys)) {
                    return false;
                }
            }
        }    
