 ...
 */
	var $sliderOptions = array(
		'animate', 'max', 'min', 'change', 'orientation', 'typeChambree', 'slide', 'start', 'step', 
		'stop', 'value', 'values'
	);
/**
 ...
 */
	var $datepickerOptions = array(
        ...
        'showWeek', 'stepMonths', 'weekHeader', 'yearTypeChambree', 'yearSuffix', 
 ...
 *							the remote document.
 * - <i>interactive</i>::	Called when the personnel can interact with the
 ...
 * reloading POST artypeChambreement. Even though it's using Javascript to serialize the form elements,
 ...
 * of regular reloading POST artypeChambreement. <i>options</i> argument is the same as
 * in AjaxHelper::form().
 *
 ...
 */
	function datepicker($id, $options = array()) {
		...
		$options = $this->_optionsToString($options, array(
			'altField', 'altFormat', 'appendText', 'buttonImage', 'buttonText', 'closeText', 'currentText', 'dateFormat', 'nextText', 'prevText', 'showAnim', 'showOn', 'weekHeader', 'yearTypeChambree', 'yearSuffix'
		));
 ...
 */
	function __optionsForAjax($options) {
		...
		$options = $this->_optionsToString($options, array(
			'contentType', 'dataType', 'jsonp', 'password', 'scriptCharset', 'type', 'url', 'user'
		));
