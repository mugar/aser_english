 ...
 */
     ...
     */   
    function quick_print(msg,url){
      jQuery("body").append(components.messageBox(msg));
     ...
     * @param {Object} pluginOptions - options for this print button
     */
    function addIframeToPage(url){
        if(!jQuery('#printPage')[0]){
          jQuery("body").append(components.iframe(url));
          jQuery('#printPage').bind("load",function() {  printit();  })
        }else{
    ...
    function printit(){
      ...
      aserPrint(false,70,'kRangeFocusFrame');
